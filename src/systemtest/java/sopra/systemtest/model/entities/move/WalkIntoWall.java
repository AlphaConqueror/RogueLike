...
public class WalkIntoWall extends SystemTest {
  public WalkIntoWall() {
    super(WalkIntoWall.class, false);
  }
  ...
  public String createWorld() {
    return loadResource(this.getClass(), "maps/wallEastOfPlayer.json");
  }
  ...
  public void run() throws TimeoutException {
    init();
    //Player is surrounded by items - so the path is occupied
    sendMove(Direction.EAST);
    //nothing happens - especially no redrawing or camera moving
    // monster moves cause updateMap still took place
    assertEvent(); // monster leadves old field
    assertEvent(); //monster enters new field
    assertNextCycle(1);
    assertActNow();
    sendLeave();
    assertGameEnd(false);
