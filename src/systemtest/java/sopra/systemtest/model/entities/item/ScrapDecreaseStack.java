...
public class ScrapDecreaseStack extends SystemTest {
  public ScrapDecreaseStack() {
    super(ScrapDecreaseStack.class, false);
  }
  ...
  public void run() throws TimeoutException {
    init();
    // pick up surrounding items - potion 1
    sendPickUp(Direction.EAST);
    assertUpdateWorld("{\"position\":{\"x\":6,\"y\":-21,\"z\":15},\"representation\":.}");
    assertNextCycle(1);
    assertActNow();
    // pick up surrounding items - potion 2
    sendPickUp(Direction.SOUTH_WEST);
    assertUpdateWorld("{\"position\":{\"x\":4,\"y\":-20,\"z\":16},\"representation\":.}");
    assertNextCycle(2);
    assertActNow();
    // scrap the potion in slot 1
    sendScrap(1);
    assertNextCycle(3);
    assertActNow();
